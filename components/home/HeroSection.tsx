...
export default function HeroSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Automatically reveal words in the H1 on load
      ...
      // Set initial states for posters to look like 3D trapezoids
      ...
      // 2. Scroll Animation (NO pin — using CSS sticky instead)
      ...
      // Transform posters from trapezoids to flat carousel
  ...
  const renderWords = (text: string) => {
    return text.split(" ").map((word, i) => (
      <span key={i} className="word opacity-0 inline-block translate-y-[50px]" style={{ marginRight: "0.3em" }}>
        {word}
      </span>
    ));
  };
  ...
  return (
    <div ref={outerRef} className="relative h-[250vh] bg-[#0a0a0a]">
      {/* Sticky inner container replaces GSAP pin */}
      <div ref={stickyRef} className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        {/* Carousel Grid Layer */}
        <div className="absolute inset-x-0 bottom-[4vh] md:bottom-[6vh] z-30 w-full max-w-[85rem] mx-auto px-4 md:px-8">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-4 md:gap-8 perspective-[1000px] h-full">
            ...
            >
              ...
              <div className="absolute bottom-6 left-6 md:left-8 z-20 w-full pr-8">
                <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.3em] text-[#888] mb-2 block font-mono">{t("Hero.residency")}</span>
                ...
                <div className="mt-4 flex justify-between items-center text-[9px] font-mono tracking-widest text-[#666] uppercase">
                  <span>{t("Hero.monday")}</span>
                  <span>{t("Hero.from")} 23:30</span>
                </div>
              </div>
            </div>
