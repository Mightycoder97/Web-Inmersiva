  ...
  return (
    <div className="pt-20">
      <div className="relative h-[60vh] md:h-[70vh] overflow-hidden">
        <Image src={event.flyer} alt={event.titulo} fill className="object-cover" priority />
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-bg-primary)] via-[rgba(10,10,10,0.5)] to-transparent" />
        <Link href="/eventos" className="absolute top-24 left-4 md:left-8 z-10 glass rounded-full px-4 py-2 flex items-center gap-2 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">
      ...
      <div className="container-custom -mt-32 relative z-10 pb-20">
        <div className="max-w-3xl">
          ...
          <h1 className="font-display text-4xl md:text-6xl lg:text-7xl font-bold uppercase tracking-tight mb-6">{event.titulo}</h1>
          ...
          <p className="text-lg text-[var(--color-text-secondary)] leading-relaxed mb-10">{event.descripcion}</p>

          <div className="mb-10">
            <h3 className="text-xs uppercase tracking-widest text-[var(--color-text-muted)] mb-4">Line Up</h3>
            <div className="flex flex-wrap gap-3">
              {event.artistas.map((artist) => (
                <span key={artist} className="px-4 py-2 glass rounded-full text-sm font-medium">{artist}</span>
              ))}
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4">
            <Button variant="primary" size="lg" href="/reservas">{t("EventDetail.bookTable")}</Button>
            <Button variant="secondary" size="lg" href="https://wa.me/51999999999">{t("EventDetail.whatsappConsult")}</Button>
          </div>
        </div>
      </div>
    </div>
