...
export default function EventosPage() {
  ...
  const months = t.raw("EventsPage.months") as string[];
  const eventTypes = t.raw("EventsPage.types") as string[];
  ...
  const filtered = mockEvents.filter((event) => {
    ...
    const typeMatch =
      selectedType === eventTypes[0] || event.tipoEvento === selectedType;
      ...
      <div className="container-custom">
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((event) => {
            ...
            return (
              <Link href={`/eventos/${event.slug}`} key={event.id} className="event-card group relative aspect-[3/4] rounded-[var(--radius-lg)] overflow-hidden block">
                <Image src={event.flyer} alt={event.titulo} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                ...
                <div className="absolute top-4 left-4 glass rounded-[var(--radius-md)] px-3 py-2 text-center">
                  <span className="block text-2xl font-bold font-display">{date.day}</span>
                  <span className="block text-[10px] uppercase tracking-wider text-[var(--color-accent)]">{date.month}</span>
                </div>
                <div className="absolute top-4 right-4">
                  <span className="px-3 py-1 text-[10px] uppercase tracking-wider font-semibold rounded-full bg-[var(--color-accent)] text-[var(--color-bg-primary)]">{event.tipoEvento}</span>
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="font-display text-xl font-bold uppercase tracking-tight mb-2 group-hover:text-[var(--color-accent)] transition-colors">{event.titulo}</h3>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20">
            <p className="text-[var(--color-text-muted)] text-lg">{t("EventsPage.noEvents")}</p>
          </div>
        )}
      </div>
    </div>
